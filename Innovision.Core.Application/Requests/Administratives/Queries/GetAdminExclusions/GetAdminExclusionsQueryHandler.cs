...
public class GetAdminExclusionsQueryHandler(ICoreDbContext coreDbContext, IMapper mapper) : IRequestHandler<GetAdminExclusionsQuery, AdministrativeExclusionVm>
{
    private readonly ICoreDbContext _coreDbContext = coreDbContext;
    private readonly IMapper _mapper = mapper;

    public async Task<AdministrativeExclusionVm> Handle(GetAdminExclusionsQuery request, CancellationToken cancellationToken)
    {
        var query = _coreDbContext.AdministrativeExclusions
            .Include(m => m.Account)
        ...
        var result = await query.ProjectTo<AdministrativeExclusionDto>(_mapper.ConfigurationProvider).ToListAsync(cancellationToken);

        return new AdministrativeExclusionVm(result)
        {
            Total = total,
