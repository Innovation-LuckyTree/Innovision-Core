...
namespace Innovision.Core.Application.Requests.Users.Queries.GetPaginateVerifiedUser
{
    public record GetPaginateVerifiedUserQuery(Guid CompanyId, PagedQuery? PagedQuery) : IRequest<ApiResponse<SystemUserVm>>;
    public class GetPaginateVerifiedUserQueryHandler(ICoreDbContext dbContext, IMapper mapper, ICurrentUserService currentUserService, ILoadingAccountServices loadingAccountServices)
        : IRequestHandler<GetPaginateVerifiedUserQuery, ApiResponse<SystemUserVm>>
    {
        ...
        public async Task<ApiResponse<SystemUserVm>> Handle(GetPaginateVerifiedUserQuery request, CancellationToken cancellationToken)
        {
            try
            {
                var query = _dbContext.Accounts
                    .Include(m => m.Branch)
                    .Where(m => m.IsVerified && ((m.AccountStatusId == AccountStatus.Migrated)
                ...
                return new ApiResponse<SystemUserVm>
                {
                    Data = new SystemUserVm
                    {
                        Results = result,
                        Total = totalCount,
