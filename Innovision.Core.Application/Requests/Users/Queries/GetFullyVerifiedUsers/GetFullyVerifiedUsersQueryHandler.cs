...
public class GetFullyVerifiedUsersQueryHandler(ICoreDbContext dbContext, IMapper mapper) : IRequestHandler<GetFullyVerifiedUsersQuery, ApiResponse<UserStatusVm>>
{
    ...
    public async Task<ApiResponse<UserStatusVm>> Handle(GetFullyVerifiedUsersQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (request.DateFrom!= null && request.DateTo != null)
                query = query.Where(m => m.LastModified >= request.DateFrom && m.LastModified <= request.DateTo);
