...
public class GetPlayersUnusedQueryHandler : IRequestHandler<GetPlayersUnusedQuery, ApiResponse<GetPlayersUnusedVM>>
{
    private readonly IMapper _mapper;
    private readonly ICoreDbContext _dbContext;

    public GetPlayersUnusedQueryHandler(IMapper mapper, ICoreDbContext dbContext)
    {
        _mapper = mapper;
        _dbContext = dbContext;
    }

    public async Task<ApiResponse<GetPlayersUnusedVM>> Handle(GetPlayersUnusedQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var query = _dbContext.Accounts
                .Include(m => m.Branch)
                .Where(m => m.UserTypeId == UserContants.USER_TYPE_PLAYER && m.AccountStatusId == 5
            ...
            var userslist = await query
                .ProjectTo<GetPlayersUnusedDto>(_mapper.ConfigurationProvider)
                .ToListAsync(cancellationToken);

            return new ApiResponse<GetPlayersUnusedVM>()
            {
                Data = new GetPlayersUnusedVM
                {
                    Players = userslist,
        ...
        {
            return new ApiResponse<GetPlayersUnusedVM>() { Success = false, ErrorMessage = ex.Message };
        }
    }
