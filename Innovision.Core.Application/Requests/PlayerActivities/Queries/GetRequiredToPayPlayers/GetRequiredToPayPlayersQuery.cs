...
namespace Innovision.Core.Application.Requests.PlayerActivities.Queries.GetRequiredToPayPlayers
{
    public record GetRequiredToPayPlayersQuery(PagedQuery? PagedQuery) : IRequest<ApiResponse<InactivePlayerVm>>;
    public class GetRequiredToPayPlayersQueryHandler(ICoreDbContext dbContext, IMapper mapper) : IRequestHandler<GetRequiredToPayPlayersQuery, ApiResponse<InactivePlayerVm>>
    {
        private readonly ICoreDbContext _dbContext = dbContext;
        private readonly IMapper _mapper = mapper;
        public async Task<ApiResponse<InactivePlayerVm>> Handle(GetRequiredToPayPlayersQuery request, CancellationToken cancellationToken)
        {
            var query = _dbContext.PlayerActivities
                .Include(m => m.Account)
                    .ThenInclude(m => m.Branch)
                .Where(m => m.RequiredTopay)
                .AsQueryable();
